...
OUTPUT_DIR = Path('/export02/users/oreng/ses-b/outputs/breathing-task/diagnostics')
...
def create_diagnostic_plots(resp_data, puls_data, ext_data, subject_id, bids_times=None):
    ...
    fig.suptitle(f'sub-{subject_id} - PMU Data Diagnostic\n' +
    ...
    if bids_times is not None:
        for _, row in bids_times.iterrows():
            ax1.axvline(row['offset_seconds'], color='green', alpha=0.7, linewidth=2, 
                       linestyle='--', label='BIDS scan' if _ == 0 else '')
            ax1.text(row['offset_seconds'], ax1.get_ylim()[1] * 0.95, 
                    row['scan_name'], rotation=90, va='top', fontsize=8, color='darkgreen',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.7))
    
    ax1.set_ylabel('Respiratory\nSignal', fontsize=11, fontweight='bold')
    ax1.set_title('Panel 1: Respiratory Trace (shaded regions = detected scans)', 
    ...
    ax2.set_title('Panel 2: Pulse Oximetry Trace', 
                  fontsize=12, fontweight='bold')
    ...
    for i in range(n_scans):
        ...
        ax3.scatter(scan_vol_times, np.ones_like(scan_vol_times) * (i+1), 
                   marker='|', s=100, linewidths=2, color=colors[i % len(colors)],
                   label=f'Scan {i+1} ({len(scan_vol_times)} vols)')
    
    ax3.set_ylabel('Scan ID', fontsize=11, fontweight='bold')
    ax3.set_title(f'Panel 3: Volume Markers Grouped by Scan ({len(volume_times)} total volumes)', 
                  fontsize=12, fontweight='bold')
    ...
    if len(volume_times) > 1:
        ...
        ax4.set_title('Panel 4: Inter-Volume Intervals (peaks = scan boundaries)', 
                     fontsize=12, fontweight='bold')
        ax4.grid(True, alpha=0.3)
        
        # Add threshold lines
        median_interval = np.median(intervals[intervals < 5])  # Exclude gaps
        ax4.axhline(median_interval, color='green', linestyle='--', linewidth=2,
                   label=f'Median TR: {median_interval:.3f}s', zorder=5)
        ax4.axhline(10, color='red', linestyle='--', linewidth=2,
                   label='Gap threshold (10s)', zorder=5)
        ...
        for i in range(n_scans - 1):
            ...
            ax4.text(gap_time, gap_size * 1.5, f'{gap_size:.1f}s gap', 
    ...
    if n_scans > 1:
        ...
        ax5.plot(t_resp[resp_mask], resp_data['values'][resp_mask], 'b-', 
                linewidth=1, alpha=0.7, label='Respiratory')
        ...
        # Mark the gap
        ax5.axvline(gap_time, color='red', linewidth=3, linestyle='--', 
                   label=f'Scan boundary ({intervals[gap_idx]:.1f}s gap)', zorder=10)
        ...
        ax5.set_title(f'Panel 5: Scan Boundary Transition (Scan 1 → Scan 2)', 
                     fontsize=12, fontweight='bold')
    ...
    if len(volume_times) > 10:
        ...
        ax6.plot(t_resp[resp_mask], resp_norm, 'b-', linewidth=1, alpha=0.7, 
                label='Respiratory (normalized)')
        ...
        ax6.plot(t_puls[puls_mask], puls_norm + 1.2, 'r-', linewidth=1, alpha=0.7,
                label='Pulse (normalized)')
        ...
        for i, vt in enumerate(volume_times[vol_mask]):
            ...
            ax6.text(vt, 2.3, f'V{i+1}', ha='center', fontsize=8, color='darkblue',
                    bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.7))
        
        # Show TR
        if len(volume_times[vol_mask]) > 1:
            tr = volume_times[vol_mask][1] - volume_times[vol_mask][0]
            ax6.annotate('', xy=(volume_times[vol_mask][1], 2.5), 
                        xytext=(volume_times[vol_mask][0], 2.5),
                        arrowprops=dict(arrowstyle='<->', color='black', lw=2))
            ax6.text((volume_times[vol_mask][0] + volume_times[vol_mask][1])/2, 2.6,
                    f'TR = {tr:.3f}s', ha='center', fontsize=9, fontweight='bold',
        ...
        ax6.set_title(f'Panel 6: Volume Markers Within Scan (first 10 volumes)', 
                     fontsize=12, fontweight='bold')
    ...
    if len(volume_times) > 1:
        ...
        ax7.hist(scan_intervals, bins=50, color='blue', alpha=0.7, 
                label=f'Within-scan TRs (n={len(scan_intervals)})')
        if len(gaps) > 0:
            ax7.hist(gaps, bins=20, color='red', alpha=0.7,
                    label=f'Scan gaps (n={len(gaps)})')
        
        ax7.axvline(np.median(scan_intervals), color='green', linestyle='--', 
                   linewidth=2, label=f'Median TR: {np.median(scan_intervals):.3f}s')
        ax7.set_xlabel('Interval (seconds)', fontsize=11, fontweight='bold')
        ax7.set_ylabel('Count', fontsize=11, fontweight='bold')
        ax7.set_title('Panel 7: Distribution of Inter-Volume Intervals', 
                     fontsize=12, fontweight='bold')
...
def print_summary_stats(resp_data, puls_data, ext_data, subject_id, bids_times=None):
    ...
    if len(ext_data['volume_markers']) > 1:
        print(f"🔍 SCAN DETECTION (from volume markers):")
        vol_times = ext_data['volume_markers'] / SAMPLING_RATE
        intervals = np.diff(vol_times)
        ...
        for i in range(n_scans):
            ...
            print(f"   Scan {i+1}: {start_time:6.1f}s - {end_time:6.1f}s ({duration:5.1f}s), "
                  f"{n_vols:4d} volumes, TR={scan_tr:.3f}s")
        print()
    
    # BIDS times
    if bids_times is not None:
        print(f"🗂️  BIDS SCAN TIMES:")
        ...
        print(f"⚠️  BIDS timing data not available\n")
...
Examples:
  ...
  python visualize_pmu_raw.py 2034
        """
    )
    ...
    if bids_times is not None:
        ...
        print(f"   ⚠️  BIDS metadata not found\n")
    ...
    print(f"  2. Check if BIDS times (green) align with 5000 markers (red)")
    print(f"  3. Identify which scan marker corresponds to your target scan")
    print(f"  4. If times don't align, investigate timing mismatch\n")
