...
Usage:
    python extract_pmu_scan.py <subject_id> <scan_name>
    python extract_pmu_scan.py 2008 breath
    python extract_pmu_scan.py 2034 rest --show-full
"""
...
OUTPUT_DIR = REPO_ROOT / "outputs" / "pmu"
...
def get_bids_scan_info(subject_id, scan_name):
    ...
    try:
        ...
        print("   Using defaults: TR=2.0s, 50 volumes")
...
def extract_scan_window(resp_data, puls_data, scan_window):
    ...
    df = pd.DataFrame({"time_sec": time_segment, "respiratory": resp_segment, "pulse": puls_segment})
...
def plot_extracted_scan(
    ...
    if show_full_session and resp_full is not None:
        ...
        if scan_window:
            ax1.axvspan(
                scan_window["start_time"], scan_window["end_time"], alpha=0.3, color="yellow", label=f"{scan_name} scan"
            )
    ...
    fig.suptitle(
        f"sub-{subject_id} - {scan_name} scan ({scan_info['n_volumes']} volumes, {scan_info['duration']:.1f}s)",
        fontsize=16,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    scan_file = output_dir / f"sub-{subject_id}_{scan_name}_{PHYSIO_SESSION}_scan.png"
...
def find_physio_dir(subject_id, physio_session=None):
    ...
    raise FileNotFoundError(
        f"No Scanner_physio folder found for sub-{subject_id} in {base_dir}\n"
...
def process_subject(subject_id, scan_name="breath", show_full=False):
    ...
    print("🔍 Identifying scans from volume markers...")
    ...
    if scan_info["offset_seconds"] > pmu_duration:
        print(
            f"⚠️  WARNING: BIDS timestamp ({scan_info['offset_seconds']:.1f}s) is beyond PMU recording ({pmu_duration:.1f}s)"
        )
        print(
            f"   Gap: {scan_info['offset_seconds'] - pmu_duration:.1f}s ({(scan_info['offset_seconds'] - pmu_duration) / 60:.1f} min)"
        )
        print("   BIDS timestamp may be incorrect - will try matching by scan properties instead.\n")
        # Don't exit! Continue to property-based matching below
    else:
        print("🎯 BIDS scan time is within PMU recording window.\n")

    # Match BIDS scan to PMU scan (works even if timestamp is wrong)
    print("🎯 Matching BIDS scan to PMU scan by properties...")
    ...
    if matched_scan is None:
        ...
        for scan in scans:
            print(
                f"   Scan {scan['scan_index'] + 1}: {scan['start_time']:.1f}s, {scan['n_volumes']} volumes, {scan['duration']:.1f}s"
            )
        print("\n   Try:")
        print("   1. Verify scan properties match one of the above")
        print("   2. Check if BIDS metadata is correct")
        print(f"   3. Run: python scripts/pmu/visualize_pmu_recording.py {subject_id}")
        sys.exit(1)
    ...
    if abs(matched_scan["start_time"] - scan_info["offset_seconds"]) > 30:
        ...
        print("      Matched by scan properties instead of timestamp")
    ...
    # Extract data
    print("✂️  Extracting scan window...")
    df = extract_scan_window(resp_data, puls_data, scan_window)
    print(f"   ✓ Extracted {len(df)} samples\n")

    # Save outputs
    print("💾 Saving outputs...")
    output_dir = OUTPUT_DIR / f"sub-{subject_id}" / PHYSIO_SESSION

    # Save Parquet (not CSV)
    parquet_file = output_dir / f"sub-{subject_id}_{scan_name}_{PHYSIO_SESSION}_data.parquet"
    output_dir.mkdir(parents=True, exist_ok=True)
...
def process_all_scans(subject_id, show_full=False):
    ...
    print("🎯 Matching PMU scans to BIDS scans...\n")
    ...
    for pmu_scan in scans:
        ...
        if best_match and best_match_score < 10:  # Reasonable match threshold
            ...
            # Create plot
            plot_extracted_scan(df, scan_info, subject_id, scan_name, output_dir, show_full_session=False)

            matched_scans.append((pmu_scan["scan_index"] + 1, scan_name))
        else:
            print("⚠️  No matching BIDS scan found")
            print("   This scan may not have corresponding BIDS data\n")
    ...
    for _scan_num, scan_name in matched_scans:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python scripts/pmu/extract_pmu_scan.py 2008
  python scripts/pmu/extract_pmu_scan.py 2008 breath
  python scripts/pmu/extract_pmu_scan.py 2034 rest --show-full
...
If no scan name is provided, all scans in the PMU recording will be processed.
    ...
    parser.add_argument(
        "scan_name", nargs="?", default=None, help="Scan name to extract (if omitted, processes all scans)"
    )
    parser.add_argument("--show-full", action="store_true", help="Also plot full session with scan highlighted")
